...
void deserialize_with_callback(Archive &archive,
  ...
  // Extract our key into the callback map
  Header header;
  archive(header);

  // Linear search through the type-sequence
...
template <typename CallbackMap, typename TypeSeq> struct CallbackRegistrar {
  ...
  TypeSeq type_sequence;

  template <typename T, typename Callback>
  auto register_callback(const std::string &id, Callback &&callback) {
  ...
  auto unpack() { return std::make_tuple(callback_map, type_sequence); }
};
...
static auto make_callback_registrar(M &&m, S &&s) {
